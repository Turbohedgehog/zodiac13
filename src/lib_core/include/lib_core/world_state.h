...
// Registers T's meta and applies its properties (State -> StateComponent, Singleton ->
// flecs Singleton). Set a singleton's value after registration, not in the same event.
template <class T>
flecs::untyped_component RegisterComponent(flecs::world& world) {
  const ImmediateScope immediate(world);
  auto component = RegisterComponentMeta<T>(world);
