...
// World state as JSON: {"version": 1, "engine_version": "0.1.0", "entities": [...]}
// with embedded component values, pretty-printed for reading by eye. engine_version
// is informational only (which build made the save, for debugging) and is not
// checked on load. What is captured follows the state markers; transport is up to
// the caller.
