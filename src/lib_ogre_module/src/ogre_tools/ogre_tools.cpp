...
Ogre::ImGuiOverlay* InitImguiOverlay() {
...
void InitImgui() {
  Ogre::ImGuiOverlay *overlay = InitImguiOverlay();
  float vp_scale = Ogre::OverlayManager::getSingleton().getPixelRatio();
...
Ogre::SceneNode* LoadDemoMesh(OgreData& ogre_data) {
  ...
  auto rotation =
      Eigen::Quaternionf::Identity() * Eigen::AngleAxisf(z13::math::ToRadians(90.), Eigen::Vector3f::UnitX()) * Eigen::AngleAxisf(z13::math::ToRadians(-90.), Eigen::Vector3f::UnitY());
