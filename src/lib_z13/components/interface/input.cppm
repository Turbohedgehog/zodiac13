...
export namespace boost {
    // Выносим multi_index и все вложенные типы/шаблоны
    using namespace multi_index; 
    
    // Выносим flat_map из пространства boost::container
    namespace container {
...
struct KeyCodeAction {
  ...
  // boost::multi_index требует, чтобы элемент был EqualityComparable (итерация
  // по контейнеру из другого модуля использует operator!= итераторов, построенный
  // из operator==). Без него компиляция падает с C2678 на boost/operators.hpp.
  bool operator==(const KeyCodeAction&) const = default;
};
